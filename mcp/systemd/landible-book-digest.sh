#!/usr/bin/env bash
# Weekly audiobook digest. Thin wrapper: load the env, then exec
# digest.py (logic + unit tests live there). Secrets go in the child's
# environment, never argv. Run by the timer (Sunday morning).
set -uo pipefail
